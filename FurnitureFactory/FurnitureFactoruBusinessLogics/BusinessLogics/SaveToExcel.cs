...
namespace FurnitureFactoryBusinessLogics.BusinessLogics
{
    ...
    {
        public static void CreateDocPurchase(ExcelInfo info)
        {
...
spreadsheetDocument.WorkbookPart.AddNewPart<SharedStringTablePart>();
        ...
        public static void CreateDocFurniture(ExcelInfo info)
        {
            using (SpreadsheetDocument spreadsheetDocument =
           SpreadsheetDocument.Create(info.FileName, SpreadsheetDocumentType.Workbook))
            {
...
spreadsheetDocument.WorkbookPart.AddNewPart<SharedStringTablePart>();
                ...
                foreach (var pc in info.Furnitures)
                {
                    ...
                    foreach (var purchase in pc.Purchases)
                    {
                        InsertCellInWorksheet(new ExcelCellParameters
                        {
                            Worksheet = worksheetPart.Worksheet,
...
SharedStringItem(new Text(cellParameters.Text)));
            ...
            cell.CellValue = new
CellValue((cellParameters.ShareStringPart.SharedStringTable.Elements<SharedStringItem>().Count() - 1).ToString());
